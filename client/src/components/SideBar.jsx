...
const SideBar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  const { chats, setSelectedChat, theme, setTheme, user, navigate, createNewChat, axios, setChats, fetchUsersChats, setToken, token, loading} = useAppContext()
      ...
      <div>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
            <div onClick={()=> {setIsMenuOpen(false); navigate('/'); setSelectedChat(chat) }}
